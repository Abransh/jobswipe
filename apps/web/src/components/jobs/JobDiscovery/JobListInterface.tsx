...
export function JobListInterface({ jobs, searchQuery, filters, onApplicationUpdate }: JobListInterfaceProps) {
  ...
  const handleJobApply = useCallback(async (job: JobData) => {
    console.log('🔵 [APPLY START - LIST VIEW]', {
      jobId: job.id,
      jobTitle: job.title,
    ...
    try {
      ...
      console.log('🟡 [API CALL - LIST VIEW]', {
      ...
      console.log('🟢 [API RESPONSE - LIST VIEW]', {
      ...
      if (response.success && response.data) {
        ...
        console.log('✅ [APPLY COMPLETE - LIST VIEW]', {
          jobId: job.id,
          success: true,
      ...
      console.error('🔴 [API ERROR - LIST VIEW]', {
        jobId: job.id,
        jobTitle: job.title,
