...
async function claimApplicationHandler(
  ...
  try {
    ...
    request.server.log.error({err: error, msg: 'Claim application error:'});
    return reply.code(500).send({
      success: false,
...
async function updateProgressHandler(
  ...
  try {
    ...
    request.server.log.error({err: error, msg: 'Update progress error:'});
    return reply.code(500).send({
      success: false,
      error: 'Internal server error',
      errorCode: 'INTERNAL_ERROR',
...
async function completeApplicationHandler(
  ...
  try {
    ...
    request.server.log.error({err: error, msg: 'Complete application error:'});
    return reply.code(500).send({
      success: false,
      error: 'Internal server error',
      errorCode: 'INTERNAL_ERROR',
