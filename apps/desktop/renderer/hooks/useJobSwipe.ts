/**
 * useJobSwipe Hook for Desktop
 * Core hook managing job queue, card states, and swipe interactions
 * Enhanced for desktop environment with keyboard and mouse support
 */
...
export function useJobSwipe({
  ...
  // Device type detection - defaulting to desktop for Electron
  ...
  const createAnalytics = useCallback((
    ...
    inputMethod: 'mouse' | 'touch' | 'keyboard' = 'mouse'
  ): SwipeAnalytics => {
  ...
  const swipeLeft = useCallback(() => {
    ...
    const analyticsData = createAnalytics(
      ...
      'left',
      cardState.velocity,
      'keyboard' // Most desktop swipes are keyboard-driven
    );
  ...
  // Gesture event handlers (enhanced for desktop)
  ...
  const onGestureMove = useCallback((event: any) => {
    setCardState(prev => ({
      ...
      swipeProgress: Math.abs(event.distance.x) / config.swipeThreshold,
      lastUpdate: Date.now()
    }));
  ...
  const getAnalyticsSummary = useCallback(() => {
    ...
    return {
      ...
      inputMethodBreakdown: {
        mouse: analytics.filter(a => a.inputMethod === 'mouse').length,
        touch: analytics.filter(a => a.inputMethod === 'touch').length,
        keyboard: analytics.filter(a => a.inputMethod === 'keyboard').length
      },
      deviceBreakdown: {
